  ...
  >
    <router-link to="/" class="flex items-center gap-4">
      <img
        class="w-20 h-20"
        alt="Japanese logo"
        src="../assets/img/Japanese_icon_for_user_box.png"
      />
    ...
    <nav class="hidden md:flex gap-3 items-center flex-1 justify-end">
      ...
      <BotonBlanco v-if="tipo === 'principal'" @click="$router.push({ name: 'register' })"
        >Registrarse</BotonBlanco
      >

      <BotonBlanco v-if="tipo === 'login'">
        <i
          class="bi bi-house text-4xl cursor-pointer"
          @click="$router.push({ name: 'principal' })"
        ></i>
        <!-- Icono de casa vuelves a principal, mirar  router/index.ts, esta vinculado a homeView.vue  -->
      </BotonBlanco>

      <BotonBlanco v-if="tipo === 'game'" @click="$router.push({ name: 'principal' })"
        >Volver</BotonBlanco
      >
    ...
    <div class="md:hidden mobile-menu-wrapper">
      <div
        class="flex-1 justify-end bg-white border-t transform transition-transform duration-150 -translate-y-2 opacity-0 pointer-events-none"
        id="mobile-menu"
      >
        <router-link
          class="block px-4 py-3"
          to="/login"
          @click="document.getElementById('menu-toggle').checked = false"
